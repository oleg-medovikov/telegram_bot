SELECT 
	...
	FROM (SELECT DAY
		...
		INNER JOIN (SELECT to_char(MAX(DAY), 'DD.MM.YYYY') day, cov_02
						FROM (SELECT DAY
									,cov_02
								FROM (SELECT r.BDATE DAY
										...
										FROM PARUS.BLINDEXVALUES  d
											...
											INNER JOIN PARUS.BALANCEINDEXES bi
												ON (d.BALANCEINDEX = bi.RN)
										WHERE rf.code = '51 COVID 19'
											  AND bi.CODE ='51_cov_02')
						PIVOT (MAX(value)
									FOR POKAZATEL IN ('51_cov_02' cov_02))) dan1
						GROUP BY cov_02) list /*Выбираем все даты последних отчетов*/
			ON (list.DAY = dan.DAY
				AND list.cov_02 = dan.cov_02)	
	ORDER BY list.DAY DESC, list.cov_02 ASC
