select 'Без амбулаторного этапа амбулаторные' as [Тип ошибки], s.[УНРЗ],s.[ФИО],s.[Дата рождения],s.[СНИЛС],s.[Вид лечения],
	s.[Медицинская организация],
	s.[МО прикрепления],s.[Исход заболевания],s.[Дата исхода заболевания],
	s.[Диагноз],s.[Диагноз установлен],DATEDIFF(day,s.[Дата исхода заболевания],getdate()) as 'Дней в непонятках'
	from
(select dbo.get_Gid(idPatient) as 'Gid',* 
	...
	where [Исход заболевания] in ('Перевод пациента в другую МО',
	        ...
	        and [Вид лечения] = 'Амбулаторное лечение'
