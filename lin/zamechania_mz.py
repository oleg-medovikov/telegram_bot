...
select dbo.get_Gid(idPatient) as 'Gid',*
	...
	where [Исход заболевания] in ('Перевод пациента в другую МО',
			...
			and [Медицинская организация] in 
('СПб ГБУЗ "Городская больница №40"','ФГБОУ ВО ПСПбГМУ им. И.П.Павлова Минздрава России'
,'ФГБОУ ВО СЗГМУ им. И.И. МЕЧНИКОВА МИНЗДРАВА РОССИИ','СПб ГБУЗ "Николаевская больница"'
...
sql_bez_ambulat_level_death_covid ="""
select fr.* from 
(select dbo.get_Gid(idPatient) as 'Gid',[УНРЗ],[ФИО],[Дата рождения],[СНИЛС],[Вид лечения],
[МО прикрепления] as 'Медицинская организация', [Исход заболевания],[Дата исхода заболевания],[Диагноз],[Диагноз установлен]
    from dbo.cv_fedreg
...
"""
sql_bez_ambulat_level_death_nocovid  = """select s.Gid,s.[УНРЗ],s.[ФИО],s.[Дата рождения],s.[СНИЛС],s.[Вид лечения],
	s.[МО прикрепления] as 'Медицинская организация',s.[Исход заболевания],s.[Дата исхода заболевания],
	s.[Диагноз],s.[Диагноз установлен],DATEDIFF(day,s.[Дата исхода заболевания],getdate()) as [Дней в статусе перевода]
	from (select dbo.get_Gid(idPatient) as 'Gid',* 
	from dbo.cv_fedreg
	where [Исход заболевания] in ('Перевод пациента в другую МО',
								'Перевод пациента на амбулаторное лечение',
								'Перевод пациента на стационарное лечение')
...
sql_bez_ambulat_level_death_nocovid_old="""
select fr.* from 
(select dbo.get_Gid(idPatient) as 'Gid',[УНРЗ],[ФИО],[Дата рождения],[СНИЛС],[Вид лечения],
[МО прикрепления] as 'Медицинская организация', [Исход заболевания],[Дата исхода заболевания],[Диагноз],[Диагноз установлен]
    from dbo.cv_fedreg
...
# Это разовое 
sql_zavishie_statusy="""select s.Gid,s.[УНРЗ],s.[ФИО],s.[Дата рождения],s.[СНИЛС],s.[Вид лечения],
...
'ФГБУ ДНКЦИБ ФМБА России','СПб ГБУЗ "Городская больница Святого Великомученика Георгия"','СПб ГБУЗ "Поликлиника №98"',
'СПб ГБУЗ "Городская Мариинская больница"','СПБ ГБУЗ "Детский городской многопрофильный клинический специализированный центр высоких медицинских технологий"'
,'СПб ГБУЗ "Городская больница №33"' ,'СПб ГБУЗ "Городская больница №40"','ФГБОУ ВО ПСПбГМУ им. И.П.Павлова Минздрава России'
,'ФГБОУ ВО СЗГМУ им. И.И. МЕЧНИКОВА МИНЗДРАВА РОССИИ','СПб ГБУЗ "Николаевская больница"'
,'СПб ГБУЗ "Городская больница №20"'
	)
) as s
...
def bez_ambulat_level(a):
    ...
    #df = pd.read_sql(sql_bez_ambulat_level_death_covid,con) 
    #put_svod(df,'Нет амбулаторного этапа, умер от COVID')
    #put_excel_for_mo(df,'Нет амбулаторного этапа, умер от COVID')

    df = pd.read_sql(sql_bez_ambulat_level_death_nocovid,con) 
    put_svod(df,'Нет амбулаторного этапа, умер не от COVID')
    put_excel_for_mo(df,'Нет амбулаторного этапа, умер не от COVID')
    temp = get_dir('temp')
    files =   temp + '/' + 'отчет по разложению Нет амбулаторного этапа.xlsx' + ';' \
            + temp + '/' + 'отчет по разложению Нет амбулаторного этапа, умер не от COVID.xlsx' #+ temp + '/' + 'отчет по разложению Нет амбулаторного этапа, умер от COVID.xlsx' + ';'  
    return files
...
def zavishie_statusy(a):
    df = pd.read_sql(sql_zavishie_statusy,con)
    put_svod(df,'зависшие статусы')
    put_excel_for_mo(df,'зависшие статусы')
    return get_dir('temp') + '/' + 'отчет по разложению зависшие статусы.xlsx'
